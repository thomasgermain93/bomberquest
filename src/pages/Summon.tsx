...
const Summon: React.FC = () => {
  ...
  const handleSummonBC = (type: 'single' | 'x10' | 'x100') => {
    ...
    if (canWriteCloud) {
      ...
      saveStatsToCloud(updatedData, loadStoryProgress(), loadDailyQuests());
    }
  };
                        ...
                        <div className="w-full">
                          ...
                          {sortedBatch.length === 1 ? (
                            <div className="flex flex-col items-center">
                              <HeroRevealCard hero={sortedBatch[0]} index={0} total={1} variant="page" />
                            ...
                            <div className="grid grid-cols-5 gap-2 justify-items-center">
                              {sortedBatch.map((hero, i) => (
                                <HeroRevealCard key={hero.id} hero={hero} index={i} total={sortedBatch.length} variant="page" />
                              ))}
                            </div>
                          )}
                        </div>
